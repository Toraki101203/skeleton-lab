...
### 2026-07-07 実施 — ✅ 合格（所要 約5分）
- 方法: `supabase link --project-ref jkjzgvzrshrgajtxdmle` → `supabase db dump --linked`（schema / --data-only、DBパスワード不要・本番へ読み取りのみ）
- 復元先: ローカル Supabase（Docker、`supabase init` 済み）
- 結果: public 全11テーブル（bookings 9 / shifts 64 / shift_requests 62 等）・auth.users(4)・storage.objects(39) すべて件数一致、エラー0
- 学び: データ投入は `SET session_replication_role = replica;` を先頭に付与。Storage 実ファイルは dump 対象外（台帳のみ）
